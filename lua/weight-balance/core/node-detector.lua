...
-- Caché por buffer para almacenar la firma de las importaciones de Node y evitar ejecuciones redundantes
local buffer_cache = {}
...
function M.check_node_dependencies(callback, bufnr)
    bufnr = bufnr or 0

    local script_path = search_parser("node")
    if not script_path then
        vim.notify("No se encontró el script parser para Node en la carpeta de scripts.", vim.log.levels.ERROR,
            { title = "WeightBalance" })
    ...
    for _, line in ipairs(lines) do
        if line:match("import%s+") or line:match("require%s*%(") then
        ...
        function(result)
            vim.schedule(function()
                if result.code ~= 0 then
                    vim.notify("Error al ejecutar el analizador de Node: " .. (result.stderr or ""), vim.log.levels
                        .ERROR,
                        { title = "WeightBalance" })
                    if callback then callback(nil) end
                    return
                ...
                if not ok or not parsed then
                    vim.notify("No se pudieron parsear los resultados de las dependencias de Node.", vim.log.levels
                        .ERROR,
                        { title = "WeightBalance" })
                    if callback then callback(nil) end
                    return
